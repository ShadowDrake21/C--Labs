...
namespace Lab3_basic_task
{
    public class StudentDAO : GenericDAO<Student>, IStudentDAO
    {
        ...
        {
            var list = session.CreateSQLQuery("SELECT Students.* FROM Students JOIN Groups" + " ON Students.GroupId = Groups.Id" + " WHERE Groups.Id=" + groupId).AddEntity("Student", typeof(Student)).List<Student>();
            return list;
        }
    }
}